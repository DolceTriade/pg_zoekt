...
- `--run-reclaim --delete-fraction F` adds a delete+tombstone+merge phase after build/seal so you can measure reclaim efficiency at different delete densities.
...
6. When reclaim mode is enabled, the report also includes rows deleted, merge runtime, and bytes reclaimed so you can tune large-segment reclaim thresholds from measurements instead of guesses.
